...
class OperacaoRepository {
    ...
    Long incluir(Operacao operacao) {
        ...
        def insertSql = """
                insert into operacao (nota_negociacao, tipo_operacao, titulo, qtde,
                    valor_total_operacao, custo_medio_venda, resultado_venda)
            ...
            """
        keys = new Sql(dataSource).executeInsert insertSql
    ...
    GroovyRowResult getById(Long idOperacao) {
        ...
        def resultado = new Sql(dataSource).firstRow(['idOperacao': idOperacao], query)
        return resultado
    }

    List<GroovyRowResult> findAll(){
        def query = 'select * from operacao'
        new Sql(dataSource).rows(query)
    }
}
